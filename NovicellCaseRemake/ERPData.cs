    ...
    public class ERPData : BackgroundService
    {
        ...
        protected async override Task ExecuteAsync(CancellationToken cancellationToken) //how does cancelation token work
        {
            while (!cancellationToken.IsCancellationRequested)
            {
                ...
                var queue = await GetERPProductData();
                AddItemsToDb(queue);
        ...
        public async Task<Queue<List<ProductDTO>>> GetERPProductData() 
        {
            ...
            Queue<List<ProductDTO>> productQueue = new Queue<List<ProductDTO>>();
            ...
            //Should this be in a loop? im guessing not
            productQueue.Enqueue(products); //I created a queue to ensure for the scenerio where the background worker processes more request than the database can handle.
            return productQueue;
        }

        public async void AddItemsToDb(Queue<List<ProductDTO>> productQueue)
        {
            //Implement a queue for this
            var products = productQueue.Dequeue(); //should this be in a loop?
